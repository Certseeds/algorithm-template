...
// Input / Process / Output three-stage structure
...
// collect leaves (nodes with degree 1), excluding root unless n==1
static output_type solve(const Input &in) {
    ...
    // degree = count of outgoing edges (because graph stores both directions)
    for (int u = 1; u <= n; ++u) {
        int deg = 0;
        for (int ei = graph.head[u]; ei != -1; ei = graph.edges[ei].next) ++deg;
        if (u != 1 && deg == 1) leaves.push_back(u);
    }
